<?php 
    ...
    if($classe == 0){
        ...
        $matiere = $config->getMatiereClasse($classe);
        if(empty($matiere)){
            echo "<h3 class='alert'>Il semble qu'il n'y ait pas de matière dans cette classe.</h3>";
        }else{ ?>
            Matière : <select name='subject' id='subject' onChange='getSequenceAddNt()'>
